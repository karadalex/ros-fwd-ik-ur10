...
def inverseKin(M0_6, M_joints):
    ...
    for epoch in range(2):
        ...
        for i in range(5):
            ...
            for j in range(3):
                for k in range(4):
                    variables = equations[j,k].free_symbols
                    ...
                    if len(variables) == 1:
                        ...
                        if varToBeSolved == th[1] or varToBeSolved == th[2]: 
                            # Solving for th2, th3 takes too long!
...
if __name__ == "__main__":
    ...
    th5_1 = ikSolutions[Symbol('th5')][0].subs(knownsDict).evalf() # IGNORE
    th5_2 = ikSolutions[Symbol('th5')][1].subs(knownsDict).evalf()
    print(th5_1, th5_2)

    # th1
    print("Solutions for th1")
    th1_1 = ikSolutions[Symbol('th1')][0].subs(knownsDict).subs({'th5':th5_1}).evalf() # IGNORE
    th1_2 = ikSolutions[Symbol('th1')][0].subs(knownsDict).subs({'th5':th5_2}).evalf()
    th1_3 = ikSolutions[Symbol('th1')][1].subs(knownsDict).subs({'th5':th5_1}).evalf() # IGNORE
